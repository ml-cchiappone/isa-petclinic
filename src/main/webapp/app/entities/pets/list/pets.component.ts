...
import { EntityArrayResponseType, PetsService } from '../service/pets.service';
...
export class PetsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPets): number => this.petsService.getPetsIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IPets[] | null): IPets[] {
    const petsNew = this.pets ?? [];
    if (data) {
      for (const d of data) {
        if (petsNew.map(op => op.id).indexOf(d.id) === -1) {
          petsNew.push(d);
        }
      }
    }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.petsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
